...
class courseCSV extends Component{
  ...
  handleFormSubmit(event){
    ...
    axios.post("/api/commity/coursetype/createCSV", formData).then(response=>{
          
      this.setState({
        loading:false,
        types: response.data.course_types,
    ...
    return (
      <>
        <div className="wrapper">
          ...
          <hr/>
            { this.state.message=="success"? <AlertSuccess new={ this.state.new } total={ this.state.total }/> : null }
            { this.state.message=="error"? <AlertError/> : null }
                ...
                
{
  ...
  <thead>
  <tr>
      <td colspan="5" className="text-center" style={{ background: 'linear-gradient(45deg,#00897B,#0081bf)' }}>Available Courses</td>
      </tr>
    <tr>
